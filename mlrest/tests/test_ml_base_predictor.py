...
@pytest.mark.parametrize(
    ('data', 'np_data', 'input_shape', 'input_type', 'prediction', 'output_shape', 'prediction_proba'),
    [(f_data, np.array(f_data), (1, 3), 'float', 0, (1, 4), np.array([f_proba]))]
)
def test_BaseData(
        mocker,
        data,
...
@pytest.mark.parametrize(
    ...
    [(
        {'data': f_data,
         ...
         'prediction': 0,
         'output_shape': (1, 4),
         'prediction_proba': np.array([f_proba])}
    ),
        (
        {'data': f_data[0],
...
def test_BaseDataDict(
    ...
    np.testing.assert_equal(mock_data.prediction_proba, data_dict['prediction_proba'])
...
@pytest.mark.parametrize(
    ...
    [(f_data, np.array(f_data).astype(np.float), (1, 3), 'float', np.float, 0, (1, 4), 'float64', np.float64, np.array([f_proba])),
     (f_data, np.array(f_data).astype(np.float64), (1, 3), 'float64', np.float64, 0, (1, 4), 'float64', np.float64, np.array([f_proba])),
     (f_data[0], np.array(f_data).astype(np.float32), (1, 3), 'float32', np.float32, 0, (1, 4), 'float64', np.float64, np.array(f_proba)),
     (i_data, np.array(i_data).astype(np.int8), (1, 3), 'int8', np.int8, 0, (1, 4), 'float64', np.float64, np.array([f_proba])),
     (i_data[0], np.array(i_data).astype(np.int16), (1, 3), 'int16', np.int16, 0, (1, 4), 'float64', np.float64, np.array(f_proba))]
)
