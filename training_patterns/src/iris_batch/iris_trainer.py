...
def split_dataset(data: List[List[Any]],
                  target: List[Any]) -> Dict[str, np.ndarray]:
...
def main():
    ...
    save_helper.dump_sklearn(
        svc_pipeline, os.path.join(
            MODEL_DIR, svc_model_filename))
    save_helper.save_interface(svc_modelname,
    ...
    save_helper.dump_sklearn(
        tree_pipeline, os.path.join(
            MODEL_DIR, tree_model_filename))
    save_helper.save_interface(tree_modelname,
